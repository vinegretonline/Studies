...
![Скриншот из игры: опалиховцы, прицеливание телефоном, вспышка](docs/ingame.jpg)

*Скриншот из автоматического теста в реальном клиенте: 4 варианта одежды; моб поднял телефон после ПКМ; вспышка на экране игрока.*
...
Workflow `.github/workflows/opalikhovets-mod.yml` в корне репозитория:
- `build`: `./gradlew build`, готовый jar выкладывается артефактом;
- `smoke-test`: поднимает выделенный сервер (`runServer`) и настоящий клиент (`runClient` под Xvfb), призывает мобов через RCON, нажимает ПКМ по мобу через xdotool и проверяет, что:
  - телефон появился в руке и потом исчез,
  - кулдаун пошёл, а повторный ПКМ во время кулдауна фото не запускает,
  - на экране игрока была белая вспышка.

  Скриншоты сохраняются в артефакт `opalikhovets-smoke`.
